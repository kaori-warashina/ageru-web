...
<?php
$result = mysql_query("SELECT * FROM `item` WHERE `item_request_flg`=2 AND `item_user`=".$_GET[user_id]);
if (!$result) {
die('クエリーが失敗しました。'.mysql_error());
}
while ($row = mysql_fetch_assoc($result)) {
...
<?php print' <img src="/media/'.$row['item_image1'].'">'?>
                    <a href="#"><span><?php print$row['item_user']?></span>さん</a>
                </div> <!-- username-top -->
...
<ul class="clearfix">
<?php
$result = mysql_query("SELECT * FROM `item` WHERE `item_request_flg`=3 AND `item_user`=".$_GET[user_id]);
if (!$result) {
die('クエリーが失敗しました。'.mysql_error());
}
while ($row = mysql_fetch_assoc($result)) {
